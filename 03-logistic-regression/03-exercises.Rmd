...
```{r}
 ...
 fit.full <- lm( FE ~ ., fe )
 step.back <- stepAIC( fit.full, score = ~ 1, direction = "backward")
 
step.forw <-  stepAIC( fit.full, score = ~ 1, direction = "forward")
```

- Are they the same model? If not why?  Which is better? JUstify your answer.

They not the same model. Step backward reduced the predictors, with optimal performance.
...
fit.min.forward <- glm(formula=GermanCredit$Class ~ ., family=binomial,data=GermanCredit)
fit.min.forward
```
